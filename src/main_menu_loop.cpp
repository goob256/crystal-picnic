...
{
	Graphics::quick_draw(box->bitmap, x, y, 0);
	...
	Graphics::draw_gauge(al_map_rgb(0xff, 0xff, 0xff), x+x1, y1, remain_w, true, percent, hilight, al_map_rgb(0x00, 0xff, 0x00));
	percent = attr.mp / (float)max_mp;
	Graphics::draw_gauge(al_map_rgb(0xff, 0xff, 0xff), x+x1, y1+gauge_span, remain_w, false, percent, hilight, al_map_rgb(0x00, 0xff, 0xff));
...
tgui::TGUIWidget *Main_Menu_Loop::get_return_button()
{
...
void Main_Menu_Loop::create_bg()
{
	...
	Graphics::quick_draw(work->bitmap, 0, 0, 0);
	Shader::use(NULL);
...
void Main_Menu_Main_Loop::draw()
{
	...
	Graphics::quick_draw(profiles[main_menu_loop->get_selected_player()]->bitmap, ox+main_menu_loop->get_green_x()+profile_x, oy+main_menu_loop->get_green_y()+profile_y, 0);
	...
	Graphics::draw_gauge(al_map_rgb(0xff, 0xff, 0xff), ox+text_x, oy+text_y, text_w, true, percent, hilight, al_map_rgb(0x00, 0xff, 0x00));
	text_y += text_h + gauge_h + 1;
	percent = (float)attr.mp / max_mp;
	Graphics::draw_gauge(al_map_rgb(0xff, 0xff, 0xff), ox+text_x, oy+text_y, text_w, false, percent, hilight, al_map_rgb(0x00, 0xff, 0xff));
	text_y += text_h + gauge_h;
	percent = (float)attack / 7;
	Graphics::draw_gauge(al_map_rgb(0xff, 0xff, 0xff), ox+text_x, oy+text_y, text_w, false, percent, hilight, al_map_rgb(0xff, 0xff, 0x00));
	text_y += text_h + gauge_h;
...
void Main_Menu_Abilities_Loop::draw()
{
	...
	for (int i = 0; i < 4; i++) {
		...
		Graphics::quick_draw(bmps[i]->bitmap, ox+button_starts[i], oy+by + 1, 0);
		if (i == selected) {
			by_save = by;
		}
		if (i != 1) {
...
void Main_Menu_Items_Loop::post_draw()
{
	if (something_dropped) {
		...
		if (cfg.language == "Korean") {
			mul = General::font_scale;
		}
		else {
			mul = 1;
		}
		bool ttf_was_quick = Graphics::ttf_is_quick();
		Graphics::ttf_quick(false); // flush cache for transform
		ALLEGRO_TRANSFORM t, backup;
		al_copy_transform(&backup, al_get_current_transform());
		al_identity_transform(&t);
		al_rotate_transform(&t, dropped_string_angle);
		al_translate_transform(&t, dropped_string_pos.x*mul, dropped_string_pos.y*mul);
		al_copy_transform(&General::korean_transform, &t);
		al_compose_transform(&t, &backup);
		al_use_transform(&t);
		General::draw_text(dropped_string, al_map_rgb(0xff, 0x00, 0x00), 0, 0, ALLEGRO_ALIGN_CENTER);
		Graphics::ttf_quick(ttf_was_quick); // re-enable cache?
		al_use_transform(&backup);
		al_identity_transform(&General::korean_transform);
	}
}
...
void Main_Menu_Equip_Loop::draw()
{
	...
	int anim_center_x = green_x + win_w / 6 * 2 / 3 + 4;
	...
	for (int i = 0; i < 4; i++) {
		Graphics::draw_gauge(al_map_rgb(0xff, 0xff, 0xff), ox+attr_x, oy+attr_y+th, (list->getX()-1)-attr_x, i == 0, percents[i], gauge_color, colors[i]);
		attr_y += 4 + th;
	}
	attr_y = green_y + 10 + top_box_h;
	bool ttf_was_quick = Graphics::ttf_is_quick();
	Graphics::ttf_quick(true);
	for (int i = 0; i < 4; i++) {
